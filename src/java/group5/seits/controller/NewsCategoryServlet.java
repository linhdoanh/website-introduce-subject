 ...
 * @author ADMIN
 */
@WebServlet(name = "NewsCategoryServlet", urlPatterns = {"/categoryNews"})
public class NewsCategoryServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            List<NewsCategoryDTO> listCateNews = dao.loadAllCate();
            request.setAttribute("LIST_NEWS_CATE", listCateNews);
            
            List<NewsDTO> listNewsById = dao.getNewsById(id);
            request.setAttribute("LIST_NEWS_BY_ID", listNewsById);
            
            RequestDispatcher rd = request.getRequestDispatcher("NewsSearch.jsp");
            rd.forward(request, response);
        } catch (SQLException ex) {
            log("NewsCategoryServlet SQL :" + ex.getMessage());
        } catch (NamingException ex) {
            log("NewsCategoryServlet Naming: " + ex.getMessage());
        } finally {
